    ...
    <tbody>
        @foreach ($histories as $key => $history)
            <tr>
                ...
                <td class="text-center">
                    {{ str_contains($history->receiver, '@g.us') ? 'Group' : $history->receiver }}
                </td>
                ...
                <td class="text-center fw-bold text-nowrap">
                    {{ ! is_null($history->executed_at) ? Carbon\Carbon::parse($history->executed_at)->isoFormat('Do MMMM YYYY HH:mm') : 'WAITING' }}
                </td>
            </tr>
        @endforeach
    </tbody>
