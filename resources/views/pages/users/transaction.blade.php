                ...
                <div class="row">
                    <div class="col">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="tableTransaction" class="display" style="width:100%">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($user->transactions as $transaction)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    {{ number_format($transaction->payment_code) }}
                                                </td>
                                                <td class="text-center">
                                                    @if ($transaction->product->discount_type == 'percentage')
                                                        -{{ number_format($transaction->product->price * ($transaction->product->discount / 100)) }}
                                                    @else
                                                        {{ $transaction->product->discount > 0 ? '-' . number_format($transaction->product->discount) : number_format($transaction->product->discount) }}
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                ...
                                                <td class="text-center fw-bold {{ $transaction->status == 'paid' ? 'text-success' : ($transaction->status == 'pending' ? 'text-warning' : 'text-danger') }}">
                                                    {{ $transaction->status }}
                                                </td>
                                                <td class="text-center">
                                                    <div class="d-flex justify-content-center">
                                                        <button class="btn btn-sm btn-info btnDetailOrder" data-id={{ $transaction->invoice }} data-bs-toggle="modal" data-bs-target="#detailOrder">
                                                            Detail
                                                        </button>
                                                    </div>
                                                </td>
