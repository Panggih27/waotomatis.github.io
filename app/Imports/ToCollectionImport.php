...
use App\Exports\FailuresImportedExport;
...
abstract class ToCollectionImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        if ($this->failures()->count() > 0) {
            $path = "excels/failures/" . $this->getUser() . '.xlsx';
            (new FailuresImportedExport($this->failures()))->store($path);
        }
     ...
     * @return self
     */
    public static function additionalUsers(callable $callback)
    {
